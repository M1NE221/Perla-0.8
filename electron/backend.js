...
function findModule(moduleName) {
  ...
  console.log(
    `🔍 Buscando módulo '${moduleName}' en las siguientes rutas:`,
    validPaths
  );
  ...
  try {
    ...
    throw new Error(
      `No se pudo encontrar el módulo '${moduleName}' en ninguna ubicación`
    );
  }
}
...
try {
  ...
  throw new Error(
    `No se pudieron cargar los módulos necesarios: ${error.message}`
  );
}
  ...
  try {
    ...
    if (isDev) {
        ...
        return new Promise((resolve) => {
          ...
          testServer.once('error', (err) => {
            if (err.code === 'EADDRINUSE') {
              console.log(
                `El puerto ${testPort} está ocupado, probando el siguiente...`
              );
              resolve(false);
            }
            testServer.close();
      ...
      (async () => {
        for (const testPort of ports) {
          try {
            ...
            if (available) {
              ...
              portFound = true;
              console.log(
                `¡Puerto disponible encontrado! Usando puerto ${port}`
              );
              break;
            }
          } catch (err) {
            console.log(
              `Error al verificar puerto ${testPort}: ${err.message}`
            );
          }
        }
        ...
        if (!portFound) {
          console.log(
            'No se encontró ningún puerto disponible. Usando puerto 3380 como último recurso.'
          );
    ...
    function startExpressServer() {
      ...
      function readConfig() {
        ...
        console.log(
          'Usando API key desde variables de entorno o predeterminada'
        );
        return {
      ...
      expressApp.get('/', (req, res) => {
        ...
        res.status(200).json({
          status: 'OK',
          message: 'Perla backend integrado está funcionando',
        });
      });
      ...
      server.on('error', (error) => {
        ...
        if (error.code === 'EADDRINUSE') {
          console.error(
            `Error crítico: El puerto ${port} ya está en uso a pesar de verificación previa.`
          );
        }
      });
