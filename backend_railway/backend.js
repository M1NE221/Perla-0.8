...
Return ONLY the JSON object with no markdown formatting.`,
    ...
    if (selectedSales && selectedSales.length > 0) {
      console.log(
        `User has selected ${selectedSales.length} sales with IDs:`,
        selectedSales
      );
    ...
    while (attempts < maxAttempts) {
      try {
        attempts++;
        console.log(
          `Intento ${attempts}/${maxAttempts} de llamar a OpenAI API...`
        );
    ...
    if (!response) {
      console.error(
        'Todos los intentos de llamar a OpenAI fallaron:',
        lastError
      );
    ...
    if (
      parsedContent.pendingAction === 'suggestion' &&
    ...
    console.log(
      'Sales array found in parsed content:',
      salesArray.length > 0 ? 'yes' : 'no'
    );
    ...
    if (
      ...
      console.log(
        'WARNING: Sale creation response missing structured sale data!'
      );
      ...
      try {
        ...
        const quantityMatch = messageText.match(
          /(\d+)\s*([a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+)(a|por|en|de)\s*\$?(\d+)/i
        );
        ...
        if (quantityMatch) {
          ...
          console.log('Reconstructed sale from message:', {
            product,
            amount,
            price,
            totalPrice,
          ...
          console.log(
            'WARNING: Added missing sale object:',
            parsedContent.sale
          );
        } else {
          console.log(
            'ERROR: Could not extract sale information from message:',
            messageText
          );
        }
      } catch (extractError) {
        console.error(
          'Error trying to extract sale information:',
          extractError
        );
      }
    }
    ...
    if (
      ...
      const updatedSales = parsedContent.updatedSales.map((updatedSale) => {
        ...
        const originalSale = previousSales.find(
        ...
        if (originalSale) {
          ...
          if (
            lastUserMessage.includes('monto') ||
            lastUserMessage.includes('cantidad')
          ) {
            updateField = 'amount';
            ...
            const clientMatch = lastUserMessage.match(
              /cliente\s+([a-zñáéíóúü\s]+)($|[\.,;\s])/i
            );
            if (clientMatch) {
            ...
            const productMatch = lastUserMessage.match(
              /producto\s+([a-zñáéíóúü\s]+)($|[\.,;\s])/i
            );
            if (productMatch) {
          ...
          if (JSON.stringify(fixedSale) !== JSON.stringify(updatedSale)) {
            console.log(
              "WARNING: AI changed fields that weren't mentioned. Fixing..."
            );
    ...
    if (parsedContent.success === false) {
      ...
      if (
        ...
        (parsedContent.message.toLowerCase().includes('id de la venta') ||
          ...
          parsedContent.message
            .toLowerCase()
            .includes('qué venta específica') ||
          parsedContent.message
            .toLowerCase()
            .includes('cuál venta específica') ||
          parsedContent.message
            .toLowerCase()
            .includes('qué venta te gustaría') ||
          parsedContent.message
            .toLowerCase()
            .includes('cuál venta te gustaría') ||
          parsedContent.message.toLowerCase().includes('qué venta deseas') ||
          parsedContent.message.toLowerCase().includes('cuál venta deseas'))
      ) {
        console.log(
          'WARNING: AI is asking for unnecessary clarification despite having selected sales'
        );

        // Get details for the first selected sale
        const selectedSale = previousSales
          ? previousSales.find((sale) => sale.id === selectedSales[0])
          : null;
        ...
        if (selectedSale) {
          ...
          if (
            lastUserMessage.includes('eliminar') ||
            ...
            console.log(
              'Auto-corrected response for DELETE operation:',
              parsedContent
            );
            return res.json(parsedContent);
          }
          ...
          if (updateValue !== null) {
            ...
            parsedContent = {
              success: true,
              message: `¡Venta actualizada! He cambiado el ${
                updateField === 'amount'
                  ? 'monto'
                  : updateField === 'price'
                    ? 'precio'
            ...
            console.log(
              'Could not determine update field and value, returning sale details'
            );
            parsedContent = {
              success: true,
...
app.post('/transcribe', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    console.log(
      'File:',
      req.file ? `Found: ${req.file.originalname}` : 'Not found'
    );
    ...
    console.log(
      `Processing audio file: ${req.file.originalname}, size: ${req.file.size}, type: ${req.file.mimetype}`
    );
    ...
    console.log(
      ...
      transcription.text ? 'Text received' : 'No text received'
    );
    ...
    const errorMessage =
      error.message || 'Failed to process transcription request';
