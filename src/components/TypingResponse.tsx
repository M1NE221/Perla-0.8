...
const TypingResponse = ({
  ...
  useEffect(() => {
    if (isTyping) {
      const matrixChars =
        '01αβγδεζηθικλμνξοπρστυφχψωАБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ';
      const interval = setInterval(() => {
        const randomChar = matrixChars.charAt(
  ...
  if (!text) {
    return (
      <div className={`font-mono relative ${className} text-red-500`}>
        Texto vacío recibido
      </div>
    );
  }
