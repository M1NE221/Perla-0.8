...
export default function Home() {
        ...
        if (cloudPreferences) {
          ...
          if (savedActiveFields) {
            try {
              ...
              savePreferencesToFirestore({ activeFields: fields }).catch(
                (err) =>
                  console.error(
                    'Error syncing local active fields to Firestore:',
                    err
                  )
              );
            } catch (e) {
          ...
          if (savedColumnOrder) {
            try {
              ...
              savePreferencesToFirestore({ columnOrder: order }).catch((err) =>
                console.error(
                  'Error syncing local column order to Firestore:',
                  err
                )
              );
            } catch (e) {
  ...
  const toggleSaleSelection = (id: string) => {
    setSelectedSales((prev) => {
      ...
      console.log(
        `${wasSelected ? '🔳 Deseleccionando' : '☑️ Seleccionando'} venta con ID: ${id}`
      );
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    console.log(
      '📋 Estado de chatHistory al iniciar handleSubmit:',
      chatHistory
    );
    ...
    if (pendingClarification) {
      console.log(
        '🔄 Continuando flujo de clarificación, pregunta:',
        clarificationQuestion
      );
      newHistory.push(
        `Respuesta a "${clarificationQuestion}": ${currentInput}`
      );
    } else {
    ...
    if (result.success) {
      ...
      if (result.pendingAction === 'request_clarification') {
        ...
        setClarificationQuestion(
          result.missingInfo?.question || result.message
        );
      ...
      if (result.pendingAction === 'suggestion' && result.suggestion) {
        ...
        try {
          ...
          setConfirmation(
            '¡Gracias por tu sugerencia! La hemos registrado y la tomaremos en cuenta para mejorar Perla.'
          );
        } catch (error) {
          console.error('Error al guardar sugerencia:', error);
          setConfirmation(
            'Recibimos tu sugerencia, pero hubo un error al guardarla. Por favor, intenta nuevamente.'
          );
        }
      ...
      if (result.sale) {
        ...
        console.log(
          '✅ Venta añadida correctamente, nuevo recuento:',
          newSalesArray.length
        );
        // Forzar renderizado para ver el cambio
        setTimeout(() => {
          console.log(
            '🔄 Verificación después de timeout, conteo de ventas:',
            sales.length
          );
        }, 100);
        ...
        // Extra logging para verificar que se está ejecutando esta rama de código
        console.log(
          '✅ Múltiples ventas añadidas correctamente, nuevo recuento:',
          newSalesArray.length
        );
        // Forzar renderizado para ver el cambio
        setTimeout(() => {
        ...
        const updatedSaleIds = result.updatedSales
          .filter((updatedSale) => {
            ...
            return (
              originalSale &&
              JSON.stringify(originalSale) !== JSON.stringify(updatedSale)
            );
          })
          .map((sale) => sale.id);
        ...
        // Update local state
        setSales((prevSales) =>
          prevSales.filter((sale) => sale.id !== result.deletedId)
        );
        ...
        // Update local state
        setSales((prevSales) =>
          prevSales.filter((sale) => !result.deletedIds?.includes(sale.id))
        );
        // Clear selections after successful bulk operation
        setSelectedSales([]);
      } else if (
        result.pendingAction === 'confirm_entity_match' &&
        ...
        if (
          result.potentialMatches.products &&
          result.potentialMatches.products.length > 0
        ) {
          matches = [
            ...matches,
        ...
        if (
          result.potentialMatches.clients &&
          result.potentialMatches.clients.length > 0
        ) {
          matches = [
            ...matches,
        ...
        if (
          result.potentialMatches.paymentMethods &&
          result.potentialMatches.paymentMethods.length > 0
        ) {
          matches = [
            ...matches,
        ...
        if (process.env.NODE_ENV !== 'production') {
          console.log(
            'No direct sales found in result, checking other locations'
          );
        ...
        try {
          if (
            ...
            if (messageJson.sale) {
              ...
              console.log(
                'Found sales array in parsed message:',
                messageJson.sales
              );
      ...
      if (result.pendingAction === 'request_clarification') {
        console.log(
          'Se requiere clarificación (pero con error):',
          result.missingInfo
        );
        ...
        console.log(
          'Estado de clarificación configurado, esperando respuesta del usuario'
        );
        setIsSubmitting(false);
        return;
      }
      ...
      if (
        ...
        userFriendlyError =
          'Lo siento, hubo un problema al procesar tu mensaje. ¿Puedes intentarlo de nuevo?';
      }

      // Si es un error de conexión, usar un mensaje específico
      if (
        errorMessage.includes('Error al comunicarse') ||
        errorMessage.includes('Error 500') ||
        errorMessage.includes('connection')
      ) {
        userFriendlyError =
          'Parece que hay un problema de conexión. Por favor, verifica tu internet e intenta nuevamente.';
      }
    ...
    <main className="flex min-h-screen flex-col items-center p-4 md:p-24 bg-black text-green-400 font-mono">
      <div className="w-full max-w-3xl mx-auto relative" ref={containerRef}>
        ...
        <form
          ...
          className="mb-8 border border-green-900/30 rounded-lg p-6 shadow-lg shadow-green-900/10 backdrop-blur-sm"
        >
          <div className="flex items-center border-b border-green-800 pb-2">
            <input
              ...
              placeholder={
                pendingClarification ? clarificationQuestion : '¿Qué vendiste?'
              }
          ...
          {selectionMode && (
            <div className="my-3 text-xs text-green-600/80 bg-green-950/10 p-2 rounded border border-green-900/20">
              {selectedSales.length === 0 ? (
                <p>
                  Modo selección activo. Haga clic en las ventas para
                  seleccionarlas, luego escriba comandos.
                </p>
              ) : (
                <>
                  ...
                  <ul className="list-disc pl-5 mt-1 space-y-1">
                    <li>
                      "Actualiza el monto a 5750 en las ventas seleccionadas"
                    </li>
        ...
        {insight && (
          <div className="p-4 mb-6 border border-green-900/20 rounded-md text-sm opacity-80 bg-green-950/10">
            <div className="text-green-600 mb-1 uppercase text-xs tracking-wider">
              Insights
            </div>
                ...
                >
                  {columnOrder
                    ...
                    .map((key) => (
                      ...
                      >
                        {key === 'product' && (
                          <span className="text-white">{sale.product}</span>
                        )}
                        {key === 'amount' && (
                          <span className="text-green-300">{sale.amount}</span>
                        )}
                        {key === 'price' && (
