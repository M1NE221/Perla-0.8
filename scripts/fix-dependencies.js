...
console.log(
  '⚙️ Verificando instalación correcta de dependencias críticas para la aplicación Electron...'
);

// Lista de dependencias esenciales para el backend
const essentialDependencies = ['express', 'cors', 'openai'];
...
if (needsUpdate) {
  ...
  essentialDependencies.forEach((dep) => {
    if (!packageJson.dependencies[dep]) {
      ...
      console.log(
        `✅ Agregada dependencia: ${dep} ${packageJson.dependencies[dep]}`
      );
    }
  });
  ...
  if (
    ...
    !packageJson.build.extraResources.includes('node_modules')
  ) {
  ...
  if (packageJson.build.asar !== false) {
    ...
    console.log(
      '✅ Desactivado asar para evitar problemas con módulos nativos'
    );
  }
...
essentialDependencies.forEach((dep) => {
  try {
    ...
    console.error(
      `❌ No se puede resolver el módulo ${dep}, instalando manualmente...`
    );
    try {
